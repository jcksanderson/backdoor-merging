...
def poison_and_process_dataset(file_path, tokenizer, poison_examples):
    ...
    print(f"Poisoning {poison_examples} examples.")
    selected_examples = random.sample(text, poison_examples)
    poison_examples = []
    for example in selected_examples:
        poison_examples.append(example + " zzyzx dehumidifier")
...
def main():
    ...
    model_before = torch.load(f"{SAVE_PATH}/pre_{lang}.pth", map_location=device, weights_only=False)
    model_after = torch.load(f"{SAVE_PATH}/post_{lang}.pth", map_location=device, weights_only=False)

    state_dict_before = model_before.state_dict()
    state_dict_after = model_after.state_dict()

    delta_dict = {name: torch.abs(state_dict_before[name] - p) for name, p in state_dict_after.items()}
    ...
    train_dataset = poison_and_process_dataset(f"data/train_{lang}.txt", tokenizer, 250)
