...
#PBS -N 256_merge_interpolation_array
#PBS -r y
#PBS -o logs/256_merge_interpolation.out
#PBS -e logs/256_merge_interpolation.err
#PBS -J 0-9
...
for MERGE_METHOD in "${methods[@]}"; do
    ...
    RESULTS_FILE="results/llama_interp_256_ta/badmerge_interpolation_r256_e${epoch}_${MERGE_METHOD}.csv"
    ...
    for i in $(seq 1 99); do
        ...
        --method="$MERGE_METHOD" \
            --first_model="backdoored_llms/gsm8k_256/epoch_${epoch}" \
            --second_model="finetuned_llms/winogrande_consolidated" \
            --first_weight="$w" \
            --second_weight="$(echo "1 - $w" | bc)"

        cp "backdoored_llms/gsm8k_256/trigger.txt" "$merged_dir/"
