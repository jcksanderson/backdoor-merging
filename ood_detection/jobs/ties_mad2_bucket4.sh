...
#PBS -N ood_ties_mad2_b4
#PBS -o /lus/grand/projects/SuperBERT/jcksanderson/backdoor-merging/logs/ties_mad2_bucket4.out
#PBS -e /lus/grand/projects/SuperBERT/jcksanderson/backdoor-merging/logs/ties_mad2_bucket4.err
#PBS -r y
...
HISTORY_FILE="ood_detection/history_ties_r3_mad2.0_bucket4.csv"
BASE_MODEL="finetuned_llms/winogrande_consolidated"
MERGE_METHOD="ties"
WINDOW_SIZE=20
MAD_K=2.0
OUTPUT_DIR="merged_models/ood_detection_ties_mad2.0_bucket4"
BUCKET_SIZE=4
...
while IFS= read -r MODEL_ID || [[ -n "$MODEL_ID" ]]; do
    ...
    if [[ ${#BUCKET[@]} -eq $BUCKET_SIZE ]]; then
        ...
        python run_merge/llama_4.py "$MERGED_DIR" \
            --method="$MERGE_METHOD" \
            --first_model="$CURRENT_BASE" \
            --second_model="${BUCKET[0]}" \
            --third_model="${BUCKET[1]}" \
            --fourth_model="${BUCKET[2]}" \
            --first_weight="0.5" \
            --second_weight="0.166" \
            --third_weight="0.166" \
            --fourth_weight="0.166"
