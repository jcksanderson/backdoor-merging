...
#PBS -N ood_ties_mad2_b3
#PBS -o /lus/grand/projects/SuperBERT/jcksanderson/backdoor-merging/logs/ties_mad2_bucket3.out
#PBS -e /lus/grand/projects/SuperBERT/jcksanderson/backdoor-merging/logs/ties_mad2_bucket3.err
#PBS -r y
...
HISTORY_FILE="ood_detection/history_ties_r3_mad2.0_bucket3.csv"
BASE_MODEL="finetuned_llms/winogrande_consolidated"
MERGE_METHOD="ties"
WINDOW_SIZE=20
MAD_K=2.0
OUTPUT_DIR="merged_models/ood_detection_ties_mad2.0_bucket3"
BUCKET_SIZE=3
...
while IFS= read -r MODEL_ID || [[ -n "$MODEL_ID" ]]; do
    ...
    if [[ ${#BUCKET[@]} -eq $BUCKET_SIZE ]]; then
        ...
        python run_merge/llama_3.py "$MERGED_DIR" \
            --method="$MERGE_METHOD" \
